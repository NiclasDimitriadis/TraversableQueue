...
TEMPL_PARAMS
TEMPL_SPECIALIZATION::TraversableQueue() :
    memory_ptr {new(std::align_val_t {alignment}) ContentType[length]()}, memory_span {memory_ptr.get(), length} {};
...
bool TEMPL_SPECIALIZATION::enqueue(const ContentType& enqueue_content) noexcept {
  ...
  if (slots_available) {
    this->memory_span[n_enqueues % length] = enqueue_content;
  }
