...
private:
  ...
  // order
  using checks_applied =
      type_pack::template apply_templ_to_nr_combs_t<Type_Handler, order>;
...
using checker_aggregator_check_t =
    checker_aggregator_check<Type_Handler, Aggregator, order, Ts...>::type;
...
template <template <typename...> class Monoid_Templ, size_t order,
          typename Type_Pack>
  requires param_pack::type_pack_convertible_v<Type_Pack> &&
           monoidal_class_template::check_closure_property_v<
               Monoid_Templ, order, param_pack::generate_type_pack_t<Type_Pack>>
...
private:
  ...
  // order
  using checks_applied =
      type_pack::template apply_templ_to_nr_combs_t<Monoid_Templ, order>;
  // clip first element to use as init when folding
  using fst = checks_applied::template index_t<0>;
  using tail = checks_applied::template truncate_front_t<1>;
