...
private:
  ...
  using monoid_applied =
      type_pack::template apply_templ_to_nr_combs_t<Class_Templ, order>;
  ...
  using pair_combinations =
      monoid_applied::template split_t<pair_combination_indices, 2>;
  // apply closure check to all pairs
  using combinations_checks =
      pair_combinations::template functor_map_t<closure_pair>;

public:
  // sum up check results of all combinations
  static constexpr bool value =
      combinations_checks::template fold_t<helpers::And, std::true_type>::value;
};
...
template <template <typename...> class Class_Templ,
  ...
  requires helpers::specializes_class_template_v<Class_Templ, A> &&
           ...
           helpers::constexpr_bool_value<
               Comparator<Class_Templ<Class_Templ<A, B>, C>,
...
private:
  template <typename type_pack>
  using associative_triple =
      associative_class_template<Class_Templ, Comparator, type_pack>;
  ...
  using combinations_checks =
      combinations::template functor_map_t<associative_triple>;
...
template <template <typename...> class Class_Templ,
  ...
  requires helpers::specializes_class_template_v<Class_Templ, A> &&
           helpers::constexpr_bool_value<
               Comparator<A, Class_Templ<Class_Templ<A>>>>
struct has_identity {
  static constexpr bool value =
...
public:
  // sum up check results of all combinations
  static constexpr bool value =
      checks::template fold_t<helpers::And, std::true_type>::value;
};
...
constexpr inline bool monoidal_class_template_v =
    monoidal_class_template<Class_Templ, Comparator, order, Ts...>::value;
}; // namespace monoidal_class_template
